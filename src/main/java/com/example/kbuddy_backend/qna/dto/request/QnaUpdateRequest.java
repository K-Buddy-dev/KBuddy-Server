...
public record QnaUpdateRequest(
        @Schema(description = "게시글 제목", example = "수정된 사이버베인 작가의 다음 작품은 언제 나오나요?", required = true, maxLength = 100)
        @NotBlank(message = "제목은 필수입니다")
        @Size(max = 100, message = "제목은 최대 100자까지 입력 가능합니다")
        String title,

        @Schema(description = "게시글 내용", example = "사이버베인 작가의 최근 인터뷰에서 다음 작품에 대한 예상 일정이 있었는지 알고 싶습니다.", required = true)
