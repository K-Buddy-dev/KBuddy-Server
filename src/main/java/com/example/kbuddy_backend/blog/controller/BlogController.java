...
public class BlogController {
	...
	@Operation(summary = "Blog 게시글 작성", description = "새로운 블로그 게시글을 작성합니다. blogSaveRequest 는 JSON 형식의 문자열로, 이미지는 선택적으로 multipart form data 로 전송합니다. <br> 임시 저장 글은 status를 DRAFT로 설정합니다.")
	...
	@Operation(summary = "Blog 게시글 업데이트", description = "블로그 게시글을 업데이트 합니다. status, 이미지 추가/삭제 등을 포함합니다. <br> 임시 저장에서 게시글로 변경 시 status를 PUBLISHED로 설정합니다.")
	...
	public ResponseEntity<Void> deleteBlog(@PathVariable final Long blogId,
		@Parameter(hidden = true) @CurrentUser User user) {
	...
	public ResponseEntity<Void> addBookmark(@PathVariable final Long blogId,
		@Parameter(hidden = true) @CurrentUser User user) {
	...
	public ResponseEntity<Void> removeBookmark(@PathVariable final Long blogId,
		@Parameter(hidden = true) @CurrentUser User user) {
	...
	public ResponseEntity<Void> plusBlogHeart(@PathVariable Long blogId,
		@Parameter(hidden = true) @CurrentUser User user) {
	...
	public ResponseEntity<Void> minusBlogHeart(@PathVariable Long blogId,
		@Parameter(hidden = true) @CurrentUser User user) {
